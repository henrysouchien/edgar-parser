...
def get_financials(
    ...
    if source == "8k":
        from .earnings_8k import get_financials_from_8k

        return get_financials_from_8k(ticker, year, quarter, full_year_mode)

    try:
        from .compat import run_edgar_pipeline
...
def get_filing_sections(
    ...
    try:
        try:
            section_parser_module = importlib.import_module("edgar_parser.section_parser")
            ...
            if exc.name != "edgar_parser.section_parser":
                raise
            module_path = Path(__file__).resolve().parent / "section_parser.py"
            if not module_path.exists():
                raise
            spec = importlib.util.spec_from_file_location("edgar_parser.section_parser", str(module_path))
